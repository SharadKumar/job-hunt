# modern-columns — per-template quality-check overrides
...
None. The active `modern-columns` template renders presentation PDF via HTML/CSS + Playwright and ATS docx via the shared `_ats-docx.ts` path, so browser-chrome and ATS-lint checks both apply.
...
- **auto_fix**: set `omitImpactHeading: true` and order summary → impact → skills in the modern-columns template design.
- **severity**: warn
...
- **auto_fix**: set modern-columns `sectionOrder` to summary → impact → skills → experience.
- **severity**: warn
...
- **auto_fix**: set `omitSummaryHeading: true` in the modern-columns template design.
- **severity**: warn
