...
1. Read `state/profile/channels.yaml` (which channels are enabled) and `state/profile/resumes.yaml` (active targets, whose `search_keywords` drive what each channel searches for).
