...
2. For each enabled channel, run `npm run hunt:<channel> -- --upsert` (`hunt:seek`, `hunt:linkedin_jobs`, ...). The channel module reads keywords from `channels.yaml` (else from active resumes, filtered by each resume's `preferred_channels`), scrapes, dedups, and upserts to `opportunities.json` with `status: discovered`. LinkedIn cards carry no JD, so `hunt:linkedin_jobs -- --upsert` also opens each new ad for the full description and `applyMethod` (up to `enrich_limit` per run); when the run reports rows left unenriched, run `npm run linkedin:enrich -- --status discovered --limit N` before classifying, because a row without a JD cannot be classified honestly. Scoring depends on the agent classification you do next.
3. **Classify each new opportunity in-agent.** For each opportunity with `status: discovered` and no classification yet, read the JD + `resumes.yaml` + profile context, produce a `Classification` matching the schema at `npm run classify:schema`. Key fields:
