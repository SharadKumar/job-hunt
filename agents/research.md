...
Steps:
1. Load the opportunity from `opportunities.json`.
...
6. Save the brief. Mark the opportunity's `interview_brief_at` timestamp in `opportunities.json`.
...
Steps:
1. Read all opportunities discovered in the last 30 days from `opportunities.json`.
...
- Two clearly different "company personas" in the search results (e.g., consultancy vs end-user) → ask: "Confirm this is <Company X> at <URL>?"
- Interview brief: more than one likely interview format (panel / take-home / live coding) → ask: "Focus brief on panel-format (most common for this role-family) / take-home-prep / live exercise / all?"
- Market mode: < 10 data points for a role family → ask: "Show partial findings with low-confidence caveat / widen scope to 60 days / skip?"
