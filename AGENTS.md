...
**Attended lane.** Every other channel, every external ATS, every recruiter or hiring-manager contact, every LinkedIn comment or DM, and every irreversible form action outside the one-click adapters requires the person present and approving that exact package at the action point. An `approve` on a non-autopilot row authorises preparation only, wherever it was tapped: the local UI (`npm run ui`) is the primary approval surface, the Google Sheet Tray is the same decision on the phone while `sheet.enabled` is true, and neither is authority to send. Recruiter and hiring-manager email is always a draft saved in the opportunity archive, never sent.
