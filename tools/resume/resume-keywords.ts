...
/**
 * The composite scorer lives in `ats-composite.ts`: it reads a finished plan
 * and a composition, and touches nothing the extraction half owns. Re-exported
 * here so every existing importer of `resume-keywords.ts` keeps resolving.
 */
export { ATS_WEIGHTS, wholeWordHit, computeAtsComposite, type AtsCompositeOptions } from "./ats-composite.ts";
