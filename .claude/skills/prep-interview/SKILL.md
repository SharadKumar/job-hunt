---
name: prep-interview
description: Produce a one-page interview brief for an upcoming interview: company snapshot, 3 to 5 STAR scenarios from the user's CV, likely questions, questions to ask. Use when the user mentions "interview tomorrow", "prep for X interview", "got an interview with company Y", "brief me for this one", "interview prep", or supplies an opportunity-id with interview-prep intent. Argument is the opportunity-id.
---
...
- **STAR scenarios** (3 to 5): drawn from `state/profile/cv-source.md`, mapped to likely themes for this opportunity. Each in the user's voice (calibrated against `state/profile/voice-samples.md`), slop- and voice-checked.
- **Likely questions** (6 to 10): technical + behavioural for this seniority + role family.
