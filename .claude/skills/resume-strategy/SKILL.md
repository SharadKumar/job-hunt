...
3b. Cloud refresh — the standing market research step, not an optional extra.
   ...
   **You refresh a CLOUD, not a type.** Keyword clouds live once in `state/org/keyword-clouds.yaml` — capability clouds (`agentic-engineering`, `enterprise-architecture`, `programme-delivery`, …), domain clouds (`financial-services-lending`, `nsw-government`, …) and tooling clouds (`claude-and-anthropic`, `azure-and-microsoft-ai`, …). A resume type does not own vocabulary; it references clouds with a weight:
   ...
   So refreshing one cloud moves **every positioning that references it** at once. Before you refresh, say which types are affected (grep `state/org/resume-types.yaml` for the cloud id) and check the weight each gives it — a cloud that is weight 5 somewhere deserves more care than one that is weight 2 everywhere. Conversely, never fix a gap for one positioning by forking a term into a second cloud: a term belongs to exactly one cloud, and the validator enforces it.
...
3c. Evidence interview (standing gap-filling; runs at the end of every cloud refresh):
   ...
   - The keyword plan already groups and orders its `questions[]` by cloud weight then JD frequency, so work through them cloud by cloud: name the cloud, ask its questions, then move to the next. Ask in batches of at most 4 with the four reusable answers "Confirm and update source", "Bring in as familiarity", "Not applicable", and "Unsure / keep pending". "Bring in as familiarity" is for a term the user did not deliver but can credibly prepare and speak to: record `--status familiarity`, apply no source patch, and the plan marks it `preppable` with `render_as: "familiarity"` — rendered once as a familiarity-framed skills line and listed under `interview_prep_terms`, never as delivered work. Each question names the term, the roles where it plausibly occurred, why the market wants it (the cloud term's `why`), and a proposed one-line phrasing, so the answer is a yes or no plus optional detail. Never ask an open-ended "tell me about X".
   - For each "Confirm and update source": draft a one-bullet patch for the role in `cv-source.md`, show it ("Apply this wording", "Edit wording", "Skip"), and apply it only on approval. Record the answer in `market-confirmations.yaml` with `kind: keyword`, `origin: attended`, and the applied `source_patch`. Remind the user that the master `.docx` must carry the same fact so `/refresh-cv` does not drop it.
   - "Bring in as familiarity" is for a term the user did not deliver but can credibly prepare and speak to: record `--status familiarity`, apply no source patch, and the plan then marks it `preppable` with `render_as: "familiarity"` — renderable once as a familiarity-framed skills line and listed under `interview_prep_terms`. It is answered, so it is never re-asked.
   - "Not applicable" suppresses the term permanently; "Unsure" leaves a `pending` row that `/resume-review` and `/review-drafts` will offer again.
...
6. Ask for approval before writing:
   ...
   - For evidence-gap questions, ask only precise, source-updating questions with choices like "Confirm and update source", "Not applicable", "Bring in as familiarity", and "Unsure / keep pending". Example: "For Claude Code work, did you define reusable slash commands/subagents/hooks that governed delivery, or was use limited to ad-hoc coding assistance?"
