...
1.5. **Proactive keyword plan + evidence interview** (the standing gap-filling pass, run before spawning resume-writer, every baseline render).
   ...
   b. If `questions` is non-empty, ask them through the four fixed answers and record them exactly as `references/harness/keyword-answers.md` sets out (batches of at most 4, highest-ranked first; `record`, then `apply-patch` for each confirmed fact). Nothing in that flow is restated here.

   c. Re-run the `--proactive` command so the plan reflects the patched corpus, then pass `--keyword-plan <plan-path>` to resume-writer in step 2 as a surfacing contract.
...
5. **Ask via the active CLI's structured question tool**:
   ...
   - For each market gap, use the four fixed answers from `references/harness/keyword-answers.md`; the tool's custom/Other answer captures detail.
